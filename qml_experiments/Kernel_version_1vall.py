...
for dataset in order:
    ...
    circuits_train = train_data["circuit"]
    labels_train = np.array(labels_train, dtype=int)
    
    labels_test = test_data["label"]
    circuits_test = test_data["circuit"]
    labels_test = np.array(labels_test, dtype=int)
    
    ############# create the quantum_x_train
    quantum_x_train_ = []
    L = len(circuits_train)
    for i in range(L):
        state = QuantumState(10)
        state.set_zero_state()
        circuits_train[i].update_quantum_state(state)
        quantum_x_train_.append(state.get_vector())
    ...
    for i in range(L):
        state = QuantumState(10)
        state.set_zero_state()
        circuits_test[i].update_quantum_state(state)
        quantum_x_test_.append(state.get_vector())
    ...
    dataset_to_save.to_csv(os.path.join(folder_path, "results_circuit_1vall.csv"), index=False)